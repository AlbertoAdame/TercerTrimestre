...
public class mainJuegoGUI {
	...
	public static void main(String[] args) {
		...
		try {
			...
			/**
			 * Hay que elegir qu√© jugador va a ser cada uno, pero no se permiten jugadores repetidos
			 */
			int numJugadores =0;
			while (numJugadores < Constantes.NUM_JUGADORES - 1 ) {
				...
				if (tipo >=1 && tipo <= jugadores.size()) {//AquÌ hemos tenido que poner un <= jugadores.size, porque si no, no dejaba coger el ˙ltimo jugador
					ordenJugadores[numJugadores++]= jugadores.get(tipo-1);
					jugadores.remove(tipo-1);
				}else {
					System.out.println("Tipo no permitido. Int√©ntelo de nuevo");
				}
